...
public class ObstacleCourse {
	...
	private boolean findExit(int row, int col) {
		...
		if (possible(row, col) && course[row][col] == ' ') {
			...
			if (onBorder(row, col)) {
				...
				escaped = findExit(row, col + 1); // Right

				if (!escaped)
					escaped = findExit(row - 1, col); // Up
				if (!escaped)
					escaped = findExit(row, col - 1); // Left
				if (!escaped)
					escaped = findExit(row + 1, col); // Down
			}
			if (escaped)
	...
	// Checks if row and col position on course
	...
	// Checks if the row and col position is on
